...
import { Database, ChevronDown, Loader2, Unplug, Plus, Circle } from "lucide-react";
...
export function ConnectionStatus() {
	...
	if (isLoading) {
		return (
			<Button variant="ghost" size="sm" disabled className="h-8 gap-2 px-2.5">
		...
		return (
			<Tooltip>
				<TooltipTrigger asChild>
					<Button variant="ghost" size="sm" asChild className="h-8 gap-2 px-2.5 hover:bg-accent/80">
						<Link to="/connections">
							<Database className="size-3.5 text-muted-foreground" />
							<span className="hidden sm:inline text-xs text-muted-foreground">Add connection</span>
						</Link>
					</Button>
				</TooltipTrigger>
				<TooltipContent side="bottom" sideOffset={8}>
					<p className="text-xs">Add a database connection</p>
				</TooltipContent>
			</Tooltip>
		);
	}
				...
				>
					{activeConnection ? (
						<>
							...
							<Badge
								variant="secondary"
								className="text-[10px] px-1.5 py-0 h-4 font-medium hidden md:inline-flex"
							>
